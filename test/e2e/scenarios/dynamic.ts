...
export async function clientListChangedAutoRefresh({ transport }: TestArgs) {
...
export async function clientListChangedCapabilityGated({ transport }: TestArgs) {
...
export async function clientListChangedSignalOnly({ transport }: TestArgs) {
    const toolCalls: Array<{ error: Error | null; items: Tool[] | null }> = [];
    const promptCalls: Array<{ error: Error | null; items: Prompt[] | null }> = [];
    const resourceCalls: Array<{ error: Error | null; items: Resource[] | null }> = [];
...
export async function mcpserverHandleEnableDisable({ transport }: TestArgs) {
...
export async function mcpserverListChangedDebounce({ transport }: TestArgs) {
    let server!: McpServer;
    const makeServer = () => {
...
export async function mcpserverRegisterPostConnect({ transport }: TestArgs) {
    let server!: McpServer;
    const makeServer = () => {
        server = new McpServer({ name: 's', version: '0' });
...
export async function mcpserverReachThroughSetRequestHandler({ transport }: TestArgs) {
