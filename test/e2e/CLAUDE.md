...
```bash
npx vitest run test/e2e/matrix.test.ts             # all
npx vitest run test/e2e/matrix.test.ts -t 'tools:' # one area
npx vitest run test/e2e/coverage.test.ts           # gate: every req has tests[]
```
